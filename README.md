...
4. Copy the generated Linux install command. It already contains `--api-base` and
   `--enrollment-token`.
...
`--api-base` is the MNSCloud API/control-plane origin. `--enrollment-token` is not typed manually
from memory; it is generated by the API and shown by the app only once.
...
.\scripts\install-agent-windows.ps1 `
  -ApiBase "https://dev.publichost.cloud" `
